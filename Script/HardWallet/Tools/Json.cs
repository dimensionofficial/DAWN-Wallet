...
public class Json
{
	...
	private const int TOKEN_VECTOR = 12;
	...
	protected static object parseValue( char[] json, ref int index, ref bool success )
	{
		switch( lookAhead( json, index ) )
		{
			...
			case Json.TOKEN_VECTOR:
				return parseVector3 (json, ref index);
...
*/
	...
	protected static Vector3 parseVector3( char[] json, ref int index )
	{
		eatWhitespace( json, ref index );
		index++;
		int lastIndex = getLastIndexOfVector( json, index );
		int charLength = ( lastIndex - index ) + 1;
		char[] numberCharArray = new char[charLength];

		Array.Copy( json, index, numberCharArray, 0, charLength );
		index = lastIndex + 1;
		string []s = (new string (numberCharArray)).Split('!');
		Vector3 result = Vector3.zero;
		result.x = float.Parse (s [0]);
		result.y = float.Parse (s [1]);
		result.z = float.Parse (s [2]);
		return result; 
	}

	protected static int getLastIndexOfVector( char[] json, int index )
	{
		int lastIndex;
		for( lastIndex = index; lastIndex < json.Length; lastIndex++ )
			if( "0123456789+-.eE!".IndexOf( json[lastIndex] ) == -1 )
	...
	protected static bool serializeValue( object value, StringBuilder builder )
	{
		...
		if (value == null) {
			...
			serializeVector3 ((Vector3)value, builder);
		}
		else
		{
			return false;
	...
	protected static void serializeVector3( Vector3 vec, StringBuilder builder )
	{
		builder.Append( "*" + vec.x + "!" + vec.y + "!" + vec.z); // , CultureInfo.InvariantCulture));
	}

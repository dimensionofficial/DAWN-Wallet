...
public class TokenRefresher : MonoBehaviour, IRefresher
{

    private static TokenRefresher instance;
    public static TokenRefresher Instance
    {
        get
        {
            if (instance == null)
            {
                ...
                o.name = "TokenRefresher";
                instance = o.AddComponent<TokenRefresher>();
            }
            return instance;
        }
    }
    ...
    {
        int page = PlayerPrefs.GetInt(address + "his_new_token1", 1);
        int pagesize = 20;
        bool needRefreshNextPage = false;
        bool tags = false;
        EthHistory.Instance.GetEthErc20History(address, 0, (j) =>
        {
            bool rr = LoomRefresher.RunAsync(() =>
            {
                var o = ParseETHRecord(j, true);
                LoomRefresher.QueueOnMainThread(() =>
                {
                    ...
                    else if (o.Count >= pagesize)
                    {
                        sqlite.InsertData(o, (r) =>
                        {
                            ...
                            {
                                PlayerPrefs.SetInt(address + "his_new_token1", page + 1);
                                needRefreshNextPage = true;
                                tags = true;
                            }
                        });
